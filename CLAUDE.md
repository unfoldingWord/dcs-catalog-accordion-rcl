...
Props are `languages`, `owners`, `subjects`, `mediaTypes`, `includeHistory`, `stage`, `dcsURL` (base host only — `API_PATH` is appended internally); the README documents them.
- **Media filtering needs history (subtle):** `mediaTypes` (`['pdf','audio','video','stream','other']`) adds `hasPdf`-style params to the language/owner/entry queries (note the casing — DCS silently ignores `hasPDF`), but a repo's media often hangs off an **older release** than its top entry — without `includeHistory` the `has*` params can match nothing. `mediaFilterParams` therefore adds `includeHistory=true` whenever media params are present (and the prop allows it), and `fetchTopCatalogEntries` then switches to a `sort=released&order=desc` history search deduped client-side to the newest row per repo (a plain `has*` search without history would silently drop repos whose media is historical).
