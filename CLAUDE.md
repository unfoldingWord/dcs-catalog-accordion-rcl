...
- **Media extensions are table-driven:** `AUDIO_EXTENSIONS`/`VIDEO_EXTENSIONS` near the top of the file decide the download section, chapter grouping, zip content detection (`_<ext>_` in a zip name), description labels and icons — add new media formats there, never in the switch statements. Chapter files (`<repo>_<lang>_vN_<chapter>_<quality>.<ext>`) roll up under a per-quality parent zip row; when the release carries chapter files without an actual zip asset (e.g. OBS-BCS/hoc_obs, 50 m4a files), the parent renders as a label-only row (no link) with the chapter expander.
...
- **TSV subject merging:** DCS models the TSV-format variant of a resource as its own `"TSV <subject>"` subject. The dropdown folds those into the base name — one merged option, counts summed across variants — and every outgoing subject list (all stats-ext/list queries and the `onFilterChange` payload) is expanded back to the concrete variants known in the universe (the `subjects` prop plus everything stats-ext has reported, tracked per-`propsSig` in `subjectVariantsRef`), so selecting `OBS Translation Notes` queries `subject=OBS Translation Notes,TSV OBS Translation Notes`. A TSV-only subject (no base form anywhere) still displays stripped but expands only to its TSV form.
